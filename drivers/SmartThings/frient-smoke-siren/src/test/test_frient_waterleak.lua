...
local mock_device                     = test.mock_device.build_test_zigbee_device(
    { profile = t_utils.get_profile_definition("water-temp-battery-siren.yml"),
      zigbee_endpoints = {
        [0x01] = {
          id = 0x01,
...
test.register_message_test(
    "Refresh Capability Command should refresh device",
    {
      ...
      },
      {
        channel = "zigbee",
        direction = "send",
        message = { mock_device.id, TemperatureMeasurement.attributes.MeasuredValue:read(mock_device):to_endpoint(0x26) }
      },
      {
        channel = "zigbee",
        direction = "send",
        message = { mock_device.id, zigbee_test_utils.build_attribute_read(mock_device, Basic.ID, { PRIMARY_SW_VERSION_ATTRIBUTE_ID }, MFG_CODE):to_endpoint(0x26) }
      }
    },
    {
