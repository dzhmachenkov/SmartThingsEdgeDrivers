...
local HEAT_FIXED_ENDIAN_SW_VERSION = "040005"
...
local function can_handle_frient(opts, driver, device, ...)
  for _, fingerprint in ipairs(FRIENT_DEVICE_FINGERPRINTS) do
    if device:get_manufacturer() == fingerprint.mfr and device:get_model() == fingerprint.model and fingerprint.subdriver == "heat" then
      return true
    end
  end
  return false
...
local generate_event_from_zone_status = function(driver, device, zone_status, zigbee_message)
  device:emit_event_for_endpoint(
      ...
      zone_status:is_alarm1_set() and capabilities.temperatureAlarm.temperatureAlarm.heat() or capabilities.temperatureAlarm.temperatureAlarm.cleared()
  )
end
...
local function primary_sw_version_attr_handler(driver, device, value, zb_rx)
  ...
  log.debug("Heat Primary Software Version firmware: 0x" .. primary_sw_version)
  device:set_field(BASE_FUNCTIONS.PRIMARY_SW_VERSION, primary_sw_version, { persist = true })
  if (primary_sw_version < HEAT_FIXED_ENDIAN_SW_VERSION) then
