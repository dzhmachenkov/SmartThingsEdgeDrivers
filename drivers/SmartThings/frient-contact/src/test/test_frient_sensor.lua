...
local mock_device2           = test.mock_device.build_test_zigbee_device(
    {
      profile = t_utils.get_profile_definition("contact-battery.yml"),
      zigbee_endpoints = {
        [0x01] = {
          id = 0x01,
          manufacturer = "frient A/S",
...
local mock_device3           = test.mock_device.build_test_zigbee_device(
    {
      profile = t_utils.get_profile_definition("contact-temperature-battery.yml"),
      zigbee_endpoints = {
        [0x01] = {
          id = 0x01,
          manufacturer = "frient A/S",
...
test.register_coroutine_test(
    ...
    function()
      test.socket.zigbee:__set_channel_ordering("relaxed")
      test.socket.capability:__queue_receive({ mock_device2.id, { capability = "refresh", component = "main", command = "refresh", args = {} } })
      test.socket.zigbee:__expect_send({ mock_device2.id, IASZone.attributes.ZoneStatus:read(mock_device2) })
...
test.register_coroutine_test(
    ...
    function()
      ...
      test.socket.device_lifecycle:__queue_receive({ mock_device2.id, "added" })
      test.wait_for_events()
      ...
      test.socket.zigbee:__expect_send(
          {
            mock_device2.id,
            IASZone.attributes.ZoneStatus:configure_reporting(mock_device2, 30, 300, 1):to_endpoint(0x23)
          }
      )
      test.socket.zigbee:__expect_send({ mock_device2.id, IASCIEAddress:write(mock_device2,
          zigbee_test_utils.mock_hub_eui) })
      test.socket.zigbee:__expect_send({ mock_device2.id, IASZone.server.commands.ZoneEnrollResponse(mock_device2,
          EnrollResponseCode.SUCCESS, 0x00) })
      test.socket.zigbee:__expect_send({ mock_device2.id, IASZone.attributes.ZoneStatus:read(mock_device2) })
...
test.register_message_test(
    "Min battery voltage report should be handled",
    {
      ...
      },
      {
        channel = "capability",
        direction = "send",
        message = mock_device2:generate_test_message("main", capabilities.battery.battery(0))
      }
    }
)
...
test.register_coroutine_test(
    ...
    function()
      test.socket.zigbee:__set_channel_ordering("relaxed")
      test.socket.capability:__queue_receive({ mock_device3.id, { capability = "refresh", component = "main", command = "refresh", args = {} } })
      test.socket.zigbee:__expect_send({ mock_device3.id, IASZone.attributes.ZoneStatus:read(mock_device3) })
      test.socket.zigbee:__expect_send({ mock_device3.id, PowerConfiguration.attributes.BatteryVoltage:read(mock_device3) })
      test.socket.zigbee:__expect_send({ mock_device3.id, TemperatureMeasurement.attributes.MeasuredValue:read(mock_device3) })
    end
)
...
test.register_coroutine_test(
    "Configure should configure all necessary attributes",
    function()
      test.socket.device_lifecycle:__queue_receive({ mock_device3.id, "added" })
      test.wait_for_events()
      ...
      test.socket.zigbee:__expect_send(
          {
            mock_device3.id,
            TemperatureMeasurement.attributes.MeasuredValue:configure_reporting(mock_device3, 30, 300,
                100)              :to_endpoint(0x23)
          }
      )
      test.socket.zigbee:__expect_send({ mock_device3.id, TemperatureMeasurement.attributes.MeasuredValue:read(mock_device3) })
      ...
      test.socket.zigbee:__expect_send(
          {
            mock_device3.id,
            IASZone.attributes.ZoneStatus:configure_reporting(mock_device3, 30, 300, 1):to_endpoint(0x26)
          }
      )
      test.socket.zigbee:__expect_send({ mock_device3.id, IASCIEAddress:write(mock_device3,
          zigbee_test_utils.mock_hub_eui) })
      test.socket.zigbee:__expect_send({ mock_device3.id, IASZone.server.commands.ZoneEnrollResponse(mock_device3,
          EnrollResponseCode.SUCCESS, 0x00) })
      test.socket.zigbee:__expect_send({ mock_device3.id, IASZone.attributes.ZoneStatus:read(mock_device3) })
    ...
    {
      {
        channel = "zigbee",
        direction = "receive",
        message = { mock_device3.id, PowerConfiguration.attributes.BatteryVoltage:build_test_attr_report(mock_device3,
            30) }
      },
      {
        channel = "capability",
        direction = "send",
        message = mock_device3:generate_test_message("main", capabilities.battery.battery(100))
      }
    }
)
...
test.register_message_test(
    "Min battery voltage report should be handled",
    {
      ...
      },
      {
        channel = "capability",
        direction = "send",
        message = mock_device3:generate_test_message("main", capabilities.battery.battery(0))
      }
    }
)
